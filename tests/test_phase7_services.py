...
def test_get_filtered_news_relevance_and_sentiment(sqlite_engine, monkeypatch) -> None:
    ...
    SessionLocal = sessionmaker(bind=sqlite_engine, autocommit=False, autoflush=False, class_=Session)
    ...
    with db_module.session_scope() as session:
        ...
        session.add(NewsMention(news_id=n1.id, company_id=comp.id, ticker="NVDA", matched_keywords="gpu"))
        session.add(NewsMention(news_id=n2.id, company_id=comp.id, ticker="NVDA", matched_keywords="gpu"))
        session.add(NewsMention(news_id=n3.id, company_id=comp.id, ticker="NVDA", matched_keywords="gpu"))
...
def test_admin_health_diagnostics(sqlite_engine, monkeypatch) -> None:
    ...
    with db_module.session_scope() as session:
        ...
        session.add(ProviderHealth(provider="gdelt", status="unhealthy", disabled_until=datetime(2026, 6, 4, 12, 0), failure_count=2))
        session.add(JobRun(job_name="refresh_news", started_at=datetime(2026, 6, 4, 10, 0), finished_at=datetime(2026, 6, 4, 10, 10), status="failed", error_text="timeout"))
